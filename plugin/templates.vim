...
function <SID>TExpandVars()
	...
	let l:user       = get(g:, 'username', executable('git') ? system('git config --global user.name') : expand('$USER'))
	let l:email      = get(g:, 'email', executable('git') ? system('git config --global user.email') : expand('$HOST'))
